...
Incoross: merge
GeneTable: random
GeneToPerformance confilicts: priority nearby search
'''
...
class TSP():
    def __init__(self, city_num, filename):
        ...
        for i in range(self.city_num):
            row = [0 for i in range(self.city_num)]
            for j in range(self.city_num):
                row[self.genTable[i, j]] = j
            self.Ttable.append(row)
        self.Ttable = np.array(self.Ttable)
    ...
    def get_ind_perfor(self, id):
        ...
        #priority nearby search
        for index in conflict:
            priority = self.genTable[index, perfor[index]]
            for i in range(1, self.city_num):
                if (priority+i >= self.city_num and priority-i < 0):
                    break
                if (priority+i < self.city_num and exist[self.Ttable[index, priority+i]] == False):
                    perfor[index] = self.Ttable[index, priority+i]
                    break
                elif (priority-i >=0 and exist[self.Ttable[index, priority-i]] == False):
                    perfor[index] = self.Ttable[index, priority-i]
                    break
