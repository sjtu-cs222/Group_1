...
class TSP():
    ...
    def ind_cross(self, id):
        ...
        exist1 = [False for i in range(self.city_num)] #record the genes in chain1
        exist2 = [False for i in range(self.city_num)]
        
        p = random.randint(1, self.city_num-1)
        for i in range(p):
            chain1.append(self.pop[id, 1, i])
            exist1[chain1[i]] = True
            chain2.append(self.pop[id, 0, i])
            exist2[chain2[i]] = True
        for i in range(p, self.city_num):
            if (exist1[self.pop[id, 0, i]] == True):
                chain2.append(self.pop[id, 0, i])
            else:
                chain1.append(self.pop[id, 0, i])
            if (exist2[self.pop[id, 1, i]] == True):
                chain1.append(self.pop[id, 1, i])
            else:
