...
Version 5
Based on: TSP_v4.py
Incoross: fetch and rearrangement
GeneTable: changable
...
class TSP():
    def __init__(self, city_num, filename):
        ...
        #生成显隐性基因表
        self.genTable = np.arange(self.city_num+1)
        np.random.shuffle(self.genTable[:self.city_num])
        for i in range(self.city_num - 1):
            row = np.arange(self.city_num+1)
            np.random.shuffle(row[:self.city_num])
    ...
    def ind_cross(self, id):
        ...
        exist = [False for i in range(self.city_num)]
        chain1 = list(self.pop[id, 1, p1:p2])
        chain2 = list(self.pop[id, 1, :p1]) + list(self.pop[id, 1, p2:])
        for gene in chain1:
            exist[gene] = True

        for i in range(self.city_num):
            if (exist[self.pop[id, 0, i]] == False):
                chain1.append(self.pop[id, 0, i])
            else:
                chain2.insert(0, self.pop[id, 0, i])
        ...
        for i in range(self.popsize):
            for j in range(2):
                if (random.random() < self.mutationrate):
                    ...
                    self.genTable[p1, self.pop[i, j, p1]] = self.genTable[p1, self.city_num]
                    self.genTable[p2, self.pop[i, j, p2]] = self.genTable[p2, self.city_num]
                    self.genTable[p1, self.city_num] += 1
                    self.genTable[p2, self.city_num] += 1
