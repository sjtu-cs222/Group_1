...
Version 3
Based on: TSP_v0.py
Incoross: partly swap https://blog.csdn.net/qq_40834030/article/details/81257099
GeneTable: random
GeneToPerformance confilicts: sequencial search
'''
...
class TSP():
    ...
    def ind_cross(self, id):
        ...
        p2 = random.randint(p1+1, self.city_num+1)

        self.pop[id, [0, 1], p1:p2] = self.pop[id, [1, 0], p1:p2]

        #deal with the conflicts in chain1
        exist = [-1 for i in range(self.city_num)] #record the swap part genes in chain1
        conflict = []   #The index of conflicted genes
        for i in range(p1, p2):
            exist[self.pop[id, 0, i]] = self.pop[id, 1, i]
        for i in range(0, p1):
            if (exist[self.pop[id, 0, i]] != -1):
                conflict.append(i)
        for i in range(p2, self.city_num):
            if (exist[self.pop[id, 0, i]] != -1):
                conflict.append(i)
        for index in conflict:
            while (exist[self.pop[id, 0, index]] != -1):
                self.pop[id, 0, index] = exist[self.pop[id, 0, index]]

        #deal with the conflicts in chain2
        exist = [-1 for i in range(self.city_num)] #record the swap part genes in chain2
        conflict = []   #The index of conflicted genes
        for i in range(p1, p2):
            exist[self.pop[id, 1, i]] = self.pop[id, 0, i]
        for i in range(0, p1):
            if (exist[self.pop[id, 1, i]] != -1):
                conflict.append(i)
        for i in range(p2, self.city_num):
            if (exist[self.pop[id, 1, i]] != -1):
                conflict.append(i)
        for index in conflict:
            while (exist[self.pop[id, 1, index]] != -1):
                self.pop[id, 1, index] = exist[self.pop[id, 1, index]]
