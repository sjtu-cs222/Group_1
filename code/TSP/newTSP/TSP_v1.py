...
Incoross: merge
GeneTable: random
GeneToPerformance confilicts: index nearby search
'''
...
class TSP():
    ...
    def get_ind_perfor(self, id):
        ...
        for index in conflict:
            now_gene = perfor[index]
            for i in range(1, self.city_num):
                if (now_gene+i >= self.city_num and now_gene-i < 0):
                    break
                if (now_gene+i < self.city_num and exist[now_gene+i] == False):
                    perfor[index] = now_gene+i
                    break
                if (now_gene-i >= 0 and exist[now_gene-i] == False):
                    perfor[index] = now_gene-i
                    break
