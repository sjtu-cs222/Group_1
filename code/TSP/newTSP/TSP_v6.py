        ...
        for i in range(self.popsize):
            for j in range(2):
                if (random.random() < self.mutationrate):
                    oldperfor = self.get_ind_perfor(i)
                    oldfit = self.get_array_fit(oldperfor)
                    for k in range(3):
                        p1 = random.randint(0, self.city_num)
                        ...
                        newperfor = self.get_ind_perfor(i)
                        newfit = self.get_array_fit(newperfor)
